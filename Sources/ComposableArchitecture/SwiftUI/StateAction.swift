...
// Experimental API
@dynamicMemberLookup
public struct StateActionProjector<StoreState, StoreAction> {
  let store: Store<StoreState, StoreAction>
  
  public subscript <Action>(dynamicMember keyPath: KeyPath<StoreState, StateAction<Action>>) -> StoreStateAction<StoreState, StoreAction, Action> {
    .init(store: store, stateAction: keyPath)
  }
}

extension Store {
  public var stateActions: StateActionProjector<State, Action> { .init(store: self) }
}

public struct StoreStateAction<StoreState, StoreAction, Action> {
  let store: Store<StoreState, StoreAction>
  let stateAction: KeyPath<StoreState, StateAction<Action>>
}
...
extension View {
  ...
  ///
  /// - Parameters:
  ///   - storeStateAction: a ``StoreStateAction`` that you extract from a store using the
  ///   `stateActions`'s space. For example `store.stateActions.$myActions`.
  ///   `KeyPath` from `State` to the `projectedValue` hosting the `StateAction`.
  ///   - perform: some action to perform when a new value is assigned to the `StateAction`.
  public func onStateAction<StoreState, StoreAction, Action>(
    _ storeStateAction: StoreStateAction<StoreState, StoreAction, Action>,
    perform: @escaping (Action) -> Void
  ) -> some View {
    self.modifier(StateActionModifier(
      store: storeStateAction.store,
      stateAction: { $0[keyPath: storeStateAction.stateAction] },
      perform: perform
    ))
  }
}
